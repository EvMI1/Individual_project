...
def send_welcome(message):
    print("START MESSAGE")
    bot.reply_to(message, "Привет! Я чат-бот, который поможет тебе найти университеты в России, предлагающие преимущесвтенно технические специальности. Ты можешь обратиться за  помощью командой /help")
...
def handle_help_command(message):
    response = "Доступные команды:\n\n" \
               ...
               "/specialty - Введите название специальности. Бот отправит Вам список университетов, в которых присутствуют интересующие Вас специальности.\n\n" \
               "/mean_score - Введите свой средний балл за ЕГЭ и минимальный балл за экзамен. Бот выведет список университетов."
...
def get_min_score(message, mean_score):
    ...
    if len(universities) > 0:
        response = "Список университетов, предлагающих специальности с средним баллом ЕГЭ ниже " + str(mean_score) + " и минимальным баллом поступления выше " + str(min_score) + ":\n"
...
def send_universities_by_specialty(message):
    ...
    specialties = [
        "Прикладная математика", "Радиотехника", "Программирование",
        "Информационная безопасность", "Фундаментальная информатика и компьютерные науки",
        "Информационные системы и технологии", "Педагогическое образование",
        "Бизнес-информатика", "Инфокоммуникационные технологии и системы связи",
        "Информационная безопасность автоматизированных систем",
        "Информационные системы и технологии", "Компьютерная безопасность",
        "Механика и математическое моделирование", "Информатика и вычислительная техника",
    ...
    bot.send_message(message.chat.id, "Выберите специальность из списка:", reply_markup=markup)
...
def get_specialty(message):
    ...
    # Отправляем сообщение с клавиатурой
    bot.send_message(message.chat.id, "Выберите университет из списка:", reply_markup=markup)
...
def get_university_info(message):
    ...
    with sqlite3.connect('universities.db') as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM Universities WHERE name LIKE ?", ('%' + selected_university + '%',))
        result = cursor.fetchone()

    # Если университет найден, отправляем информацию
    if result:
        response = f"Университет: {result[1]}\n\n" \
    ...
    # Скрываем клавиатуру после завершения операции
...
def handle_text_message(message):
    ...
    with sqlite3.connect('universities.db') as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM Universities WHERE name LIKE ?", ('%' + university_name + '%',))
        result = cursor.fetchone()
